...
def export_row(row, f):
    ...
    print >> f, ">%s\t%s\t%s\t%i\t%i\t%i\t%i\t%f\t%f\t%f\t%s\t%s\t%s" % (
        pdb_id,
        pdb_ch,
        uniprot_id,
        pdb_res_list[pdb_from],
        pdb_res_list[pdb_to],
        uniprot_from,
        uniprot_to,
        evalue,
...
def main():
    ...
    try:
        opts, args = getopt.getopt(
            sys.argv[1:], "", ["host=", "port=", "user=", "passwd=", "db=", "output="]
        )
    ...
    cursor = db.cursor()
    cursor.execute(
        "select distinct pp.pdbid, pp.chcode, mb.seqID, mb.mbegin, pmr.res, pp.query, pp.hit, pp.midline, pp.qfrom, pp.hfrom, pp.qto, pp.hto, pp.evalue, pp.identity, pp.identp "
