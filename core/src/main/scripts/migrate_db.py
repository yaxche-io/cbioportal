        ...
        if ":" in database_host:
            ...
            if self.database_host.strip() == "localhost":
                print(
                    "Invalid host config '"
                    + database_host
                    + "' in properties file. If you want to specify a port on local host use '127.0.0.1' instead of 'localhost'",
                    file=ERROR_FILE,
                )
                sys.exit(1)
...
def get_db_cursor(portal_properties):
    ...
    try:
        connection = MySQLdb.connect(
            host=portal_properties.database_host,
...
def print_all_check_reference_genome_warnings(warnings, force_migration):
    ...
    clean_up_string = (
        " Please clean up the mutation_event table and ensure it only contains references to one of the valid reference genomes (%s)."
        % (allowable_reference_genome_string)
    )
    use_default_string = (
        "the default reference genome (%s) will be used in place of invalid reference genomes and the first encountered reference genome will be used."
        % (DEFAULT_GENOME_REFERENCE)
    )
    use_force_string = (
        'OR use the "--force" option to override this warning, then %s'
        % (use_default_string)
    )
    ...
    for warning in warnings:
        if force_migration:
            print(
                "%s%s\n%s%s\n" % (indent, warning, indent, forcing_string),
                file=ERROR_FILE,
            )
        else:
            print(
                "%s%s%s\n%s%s\n"
                % (indent, warning, clean_up_string, indent, use_force_string),
                file=ERROR_FILE,
            )
...
def validate_reference_genome_values_for_study(warnings, ncbi_to_count, study):
    ...
    if len(ncbi_to_count) == 1:
        for retrieved_ncbi_build in ncbi_to_count:  # single iteration
            if retrieved_ncbi_build.upper() not in [
                x.upper() for x in ALLOWABLE_GENOME_REFERENCES
            ]:
                msg = (
                    "WARNING: Study %s contains mutation_event records with unsupported NCBI_BUILD value %s."
                    % (study, retrieved_ncbi_build)
                )
                warnings.append(msg)
    elif len(ncbi_to_count) > 1:
        msg = (
            "WARNING: Study %s contains mutation_event records with %s NCBI_BUILD values {ncbi_build:record_count,...} %s."
...
def check_reference_genome(portal_properties, cursor, force_migration):
    ...
    print(
        "Checking database contents for reference genome information", file=OUTPUT_FILE
    )
    ...
    try:
        ...
        study_to_ncbi_to_count = (
            {}
        )  # {cancer_study_identifier : {ncbi_build  : record_count}}
        for row in cursor.fetchall():
        ...
        for study in study_to_ncbi_to_count:
            validate_reference_genome_values_for_study(
                warnings, study_to_ncbi_to_count[study], study
            )
    except MySQLdb.Error as msg:
        print(msg, file=ERROR_FILE)
        sys.exit(1)
...
def run_statements(statements, connection, cursor):
    ...
    for version, statement_list in statements.items():
        ...
        for statement in statement_list:
            print("\tExecuting statement: %s" % (statement.strip()), file=OUTPUT_FILE)
            try:
...
def main():
    ...
    parser.add_argument(
        "-y", "--suppress_confirmation", default=False, action="store_true"
    )
    parser.add_argument(
        "-p",
        "--properties-file",
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="Force to run database migration",
    )
    ...
    if not os.path.exists(properties_filename):
        print(
            "properties file %s cannot be found" % (properties_filename),
            file=ERROR_FILE,
        )
        usage()
        sys.exit(2)
    ...
    if portal_properties is None:
        print(
            "failure reading properties file (%s)" % (properties_filename),
            file=ERROR_FILE,
        )
        sys.exit(1)
