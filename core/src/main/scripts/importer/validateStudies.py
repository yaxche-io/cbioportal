...
def main(args):
    ...
    possible_exit_status = {
        0: "VALID",
        1: "INVALID",
        2: "INVALID (PROBLEMS OCCURRED)",
    ...
    for study in list_studies:
        ...
        if output_folder is not None:
            ...
            # file or when there is no cancer_study_identifier and it will not create the HTML file at all.
                ...
                print(var_traceback)
                print(
                    "Validation from study "
                    + study
                    + " will not be written to HTML file"
                    + "\x1b[0m"
                )

        # Get the path to the validator , in the same directory as this script
        validation_script = os.path.join(
            os.path.dirname(os.path.realpath(__file__)), "validateData.py"
        )
        ...
        exit_status_message = possible_exit_status.get(
            exit_status_study, "Unknown status: {}".format(exit_status_study)
        )
        if exit_status_study == 0 or exit_status_study == 3:
            ...
            validation_exit_status = (
                1  # When invalid check the exit status to one, for failing circleCI
            )
...
def interface(args=None):
    ...
    parser = argparse.ArgumentParser(
        description="Wrapper where cBioPortal study validator is run for multiple studies"
    )
    parser.add_argument(
        "-d",
        "--root-directory",
        type=str,
        help="Path to directory with all studies that should be validated",
    )
    parser.add_argument(
        "-l",
        "--list-of-studies",
        type=str,
        help="List with paths of studies which should be validated",
    )
    parser.add_argument(
        "-html",
        "--html-folder",
        type=str,
        help="Path to folder for output HTML reports",
    )
