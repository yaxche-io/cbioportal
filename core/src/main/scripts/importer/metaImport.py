...
def interface():
    ...
    parser.add_argument("-html", "--html_table", type=str, help="path to html report")
    parser.add_argument(
        "-v",
        "--verbose",
        action="store_true",
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(
            Color.RED + "Error occurred during validation step:" + Color.END,
            file=sys.stderr,
        )
        raise
    finally:
    ...
    try:
        ...
        if exitcode == 1:
            ...
            if args.override_warning:
                print(
                    Color.BOLD + "Overriding Warnings. Importing study now" + Color.END,
                    file=sys.stderr,
                )
                print("#" * 71 + "\n", file=sys.stderr)
            ...
            print(
                Color.BOLD + "Everything looks good. Importing study now" + Color.END,
                file=sys.stderr,
            )
            print("#" * 71 + "\n", file=sys.stderr)
            cbioportalImporter.main(args)
        ...
        print(
            Color.BOLD
            + "\nProcess interrupted. You will have to run this again to make sure study is completely loaded."
            + Color.END,
            file=sys.stderr,
        )
        print("#" * 71, file=sys.stderr)
