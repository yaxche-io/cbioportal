...
def import_study_data(
    jvm_args,
    meta_filename,
    data_filename,
    update_generic_assay_entity,
    ...
    if (
        update_generic_assay_entity != None
        and update_generic_assay_entity.casefold() == "True".casefold()
    ):
    ...
    if meta_file_type is None:
        print(
            ("Unrecognized meta file type '%s', skipping file" % (meta_file_type)),
            file=ERROR_FILE,
        )
        return
    ...
    if not data_filename.endswith(meta_file_dictionary["data_filename"]):
        print(
            (
                "'data_filename' in meta file contradicts "
    ...
    if (
        importer == "org.mskcc.cbio.portal.scripts.ImportProfileData"
        and shouldUpdateGenericAssayEntities
    ):
        args.append("--update-info")
        args.append("True")
    elif (
        importer == "org.mskcc.cbio.portal.scripts.ImportProfileData"
        and not shouldUpdateGenericAssayEntities
    ):
        args.append("--update-info")
        args.append("False")
    if importer in (
...
def process_command(
    ...
    if command == IMPORT_CANCER_TYPE:
        ...
        if study_ids == None:
            ...
            raise RuntimeError(
                "Your command uses both -id and -meta. Please, use only one of the two parameters."
            )
    elif command == IMPORT_STUDY_DATA:
        import_study_data(
            jvm_args, meta_filename, data_filename, update_generic_assay_entity
        )
...
def process_directory(jvm_args, study_directory, update_generic_assay_entity=None):
    ...
    meta_filenames = (
        ...
        for meta_filename in os.listdir(study_directory)
        if re.search(r"(\b|_)meta(\b|[_0-9])", meta_filename, flags=re.IGNORECASE)
    ...
    for meta_filename in meta_filenames:
        ...
        if meta_file_type == MetaFileTypes.CANCER_TYPE:
            cancer_type_filepairs.append(
                (
                    meta_filename,
                    os.path.join(study_directory, meta_dictionary["data_filename"]),
                )
            )
            ...
            if resource_definition_filepair is not None:
                raise RuntimeError(
                    "Multiple resource definition files found: {} and {}".format(
                        resource_definition_filepair[0], meta_filename
                    )
            ...
            if sample_attr_filepair is not None:
                raise RuntimeError(
                    "Multiple sample attribute files found: {} and {}".format(
                        sample_attr_filepair[0], meta_filename
                    )
                )  # pylint: disable=unsubscriptable-object
            ...
            if sample_resource_filepair is not None:
                raise RuntimeError(
                    "Multiple sample resource files found: {} and {}".format(
                        sample_resource_filepair[0], meta_filename
                    )
                )  # pylint: disable=unsubscriptable-object
        ...
        # Check for z-score exression files
            ...
            meta_file_type == MetaFileTypes.EXPRESSION
            and meta_dictionary["datatype"] == "Z-SCORE"
        ):
    ...
    if (
        study_meta_dictionary[study_meta_filename]
        .get("add_global_case_list", "false")
        .lower()
        == "true"
    ):
...
def usage():
    ...
    print(
        (
            "cbioportalImporter.py --jar-path (path to scripts jar file) "
...
def add_parser_args(parser):
    ...
    parser.add_argument(
        "-jar", "--jar_path", type=str, required=False, help="Path to scripts JAR file"
    )
    parser.add_argument(
        "-meta", "--meta_filename", type=str, required=False, help="Path to meta file"
    )
    parser.add_argument(
        "-data", "--data_filename", type=str, required=False, help="Path to Data file"
    )
...
def interface():
    ...
    import_cancer_type = subparsers.add_parser(
        "import-cancer-type", parents=[parent_parser], add_help=False
    )
    import_study = subparsers.add_parser(
        "import-study", parents=[parent_parser], add_help=False
    )
    import_study_data = subparsers.add_parser(
        "import-study-data", parents=[parent_parser], add_help=False
    )
    import_case_list = subparsers.add_parser(
        "import-case-list", parents=[parent_parser], add_help=False
    )
    remove_study = subparsers.add_parser(
        "remove-study", parents=[parent_parser], add_help=False
    )

    remove_study.add_argument(
        "-id",
        "--study_ids",
        type=str,
        required=False,
...
def main(args):
    ...
    error_handler.setFormatter(cbioportal_common.LogfileStyleFormatter(os.getcwd()))
