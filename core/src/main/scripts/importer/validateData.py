...
class Jinja2HtmlHandler(logging.handlers.BufferingHandler):
    ...
    def __init__(
        self, study_dir, output_filename, max_reported_values, *args, **kwargs
    ):
...
class Validator(object):
    ...
    def _validate_file(self):
        ...
        with open(self.filename, "r", newline=None) as data_file:
            ...
            try:
                ...
                self.logger.error(
                    "Not a valid tab separated file. Check if all lines have the same number of columns and if all separators are tabs."
                )
                return
    ...
    def checkGeneIdentification(self, gene_symbol=None, entrez_id=None):
        ...
        # no Entrez gene id, only a gene symbol
            ...
            if num_entrezs_for_hugo == 1:
                # set the value to be returned
                identified_entrez_id = self.portal.hugo_entrez_map[gene_symbol][0]
            ...
            # no canonical symbol, but a single unambiguous alias
                ...
                # set the value to be returned
                identified_entrez_id = self.portal.alias_entrez_map[gene_symbol][0]
    ...
    def checkDriverAnnotationColumn(self, driver_value=None, driver_annotation=None):
        ...
        """
        if driver_annotation is None and (
            driver_value is "Putative_Driver" or driver_value is "Putative_Passenger"
        ):
            self.logger.error(
                ...
                '"Putative_Passenger".',
                extra={"line_number": self.line_number, "cause": driver_annotation},
            )
        return None

    def checkDriverTiersColumnsValues(
        self, driver_tiers_value=None, driver_tiers_annotation=None
    ):
    ...
    def load_chromosome_lengths(genome_build, logger):
        ...
        class InvalidAPIResponse(ValueError):
            def __init__(self, chrom_size_url, line):
                super().__init__(
                    "Unexpected response from {}: {}".format(chrom_size_url, repr(line))
                )
        ...
        logger.debug("Retrieving chromosome lengths from '%s'", chrom_size_url)
        ...
        for line in r.text.splitlines():
            ...
            if line.startswith("#"):
                continue
            cols = line.split("\t", 1)
            if not (len(cols) == 2 and cols[0].startswith("chr")):
...
class MutationsExtendedValidator(Validator):
    ...
    def checkHeader(self, cols):
        ...
        if "cbp_driver_annotation" in self.cols and "cbp_driver" not in self.cols:
            self.logger.error(
                ...
                "column.",
                extra={"column_number": self.cols.index("cbp_driver_annotation")},
            )
        if (
            "cbp_driver_tiers_annotation" in self.cols
            and "cbp_driver_tiers" not in self.cols
        ):
            self.logger.error(
                "Column cbp_driver_tiers_annotation "
                "found without any cbp_driver_tiers "
                "column.",
                extra={"column_number": self.cols.index("cbp_driver_tiers_annotation")},
            )
        ...
        if "cbp_driver" in self.cols and "cbp_driver_annotation" not in self.cols:
            self.logger.error(
                ...
                "column.",
                extra={"column_number": self.cols.index("cbp_driver")},
            )
        if (
            "cbp_driver_tiers" in self.cols
            and "cbp_driver_tiers_annotation" not in self.cols
        ):
            self.logger.error(
                "Column cbp_driver_tiers "
                "found without any cbp_driver_tiers_annotation "
                "column.",
                extra={"column_number": self.cols.index("cbp_driver_tiers")},
            )
    ...
    def checkAlleleMAFFormat(self, data):
        ...
        if set(necessary_columns_check10).issubset(self.cols):
            ...
            if (
                data[self.cols.index("Start_Position")] == ""
                or data[self.cols.index("End_Position")] == ""
            ):
                positions = False
            else:
                positions = True
    ...
    def checkNCBIbuild(self, value):
        if value != "":
            ...
            if self.portal.species == "human":
                if value not in [
                    str(self.portal.ncbi_build),
                    self.portal.genome_build,
                    "GRCh" + str(self.portal.ncbi_build),
                ]:
                    return False
                ...
                if value not in [
                    str(self.portal.ncbi_build),
                    self.portal.genome_build,
                    "GRCm" + str(self.portal.ncbi_build),
                ]:
                    return False
        return True
...
class ClinicalValidator(Validator):
    ...
    PREDEFINED_ATTRIBUTES = {
        ...
        "GLEASON_SCORE": {},
        "HISTOLOGY": {"datatype": "STRING",},
        "KNOWN_MOLECULAR_CLASSIFIER": {"datatype": "STRING",},
        "METASTATIC_SITE": {"is_patient_attribute": "0", "datatype": "STRING",},
        "MOUSE_STRAIN": {},
        ...
        "PRIMARY_SITE": {"is_patient_attribute": "0", "datatype": "STRING",},
        ...
        "TUMOR_GRADE": {},
        "TUMOR_SITE": {"is_patient_attribute": "1", "datatype": "STRING",},
        "TUMOR_STAGE_2009": {},
        "TUMOR_TISSUE_SITE": {"is_patient_attribute": "0", "datatype": "STRING",},
...
class FusionValidator(Validator):
    ...
    def checkLine(self, data):
        ...
        fusion_entry = "\t".join(
            [
                data[self.cols.index("Hugo_Symbol")],
        ...
        if fusion_entry in self.fusion_entries:
            self.logger.warning(
                ...
                extra={
                    "line_number": self.line_number,
                    "cause": "%s (already defined on line %d)"
                    % (fusion_entry, self.fusion_entries[fusion_entry]),
                },
            )
        else:
...
class StructuralVariantValidator(Validator):
    ...
    def checkHeader(self, data):
        ...
        if not (
            ...
            self.logger.error(
                "Structural variant requires Site1_Entrez_Gene_Id and/or Site1_Hugo_Symbol column",
                extra={"line_number": self.line_number},
            )
            num_errors += 1
        ...
        if not (
            ...
            self.logger.error(
                "Fusion event requires Site2_Entrez_Gene_Id and/or Site2_Hugo_Symbol column",
                extra={"line_number": self.line_number},
            )
            num_errors += 1
        ...
        if not {"Site1_Ensembl_Transcript_Id", "Site2_Ensembl_Transcript_Id"}.issubset(
            set(self.cols)
        ):
            self.logger.error(
        ...
        def checkFusionValues(self, data):
            ...
            def checkTranscriptPresence(self, transcript, column_name):
                ...
                if transcript is None and column_name in self.cols:
                    self.logger.error(
                        "No Ensembl transcript ID found. Please provide the Ensembl transcript ID "
            ...
            def checkExonPresence(self, exon, column_name):
                ...
                if exon is None and column_name in self.cols:
                    self.logger.error(
                        "No exon found. Please provide the exon for breakpoint visualization",
                        extra={
                            "line_number": self.line_number,
                            "column_number": self.cols.index(column_name) + 1,
                        },
            ...
            site1_transcript = checkPresenceValue(
                "Site1_Ensembl_Transcript_Id", self, data
            )
            site2_transcript = checkPresenceValue(
                "Site2_Ensembl_Transcript_Id", self, data
            )
            ...
            checkTranscriptPresence(
                self, site1_transcript, "Site1_Ensembl_Transcript_Id"
            )
            checkTranscriptPresence(
                self, site2_transcript, "Site2_Ensembl_Transcript_Id"
            )
            ...
            if None not in [site1_exon, site2_exon]:
                # Check value of exon
                site1_exon = self.parse_exon(
                    site1_exon, column_number=self.cols.index("Site1_Exon") + 1
                )
                site2_exon = self.parse_exon(
                    site2_exon, column_number=self.cols.index("Site2_Exon") + 1
                )
    ...
    def onComplete(self):
        ...
        def listAllTranscripts(self):
            ...
            for line_number in self.structural_variant_entries.keys():
                ...
                if len(self.structural_variant_entries[line_number].keys()) == 0:
                    continue
                if (
                    self.structural_variant_entries[line_number][
                        "Site1_Ensembl_Transcript_Id"
                    ]
                    is not None
                ):
                    self.transcript_set.add(
                        self.structural_variant_entries[line_number][
                            "Site1_Ensembl_Transcript_Id"
                        ]
                    )
        ...
        def retrieveTranscriptsAndExons(self):
            ...
            request_data = '{ "transcriptIds" : ["%s"] }' % (
                '", "'.join(self.transcript_set)
            )
            request = requests.post(
                url=request_url, headers=request_headers, data=request_data
            )
            if request.ok:
        ...
        def validateTranscripts(self):
            ...
            for transcript in list(self.transcript_set):
                if transcript not in self.transcript_exons_dict:
                    self.logger.error(
                        "Ensembl transcript not found in Genome Nexus.",
                        extra={"cause": transcript},
                    )
                else:
                    if self.transcript_exons_dict[transcript] is None:
                        self.logger.error(
                            "This transcript does not contain known exons in Genome Nexus. Please update"
        ...
        def validateExons(self):
            ...
            def validateExonsInTranscript(self, transcript, exon, line_number):
                ...
                if not exon_in_transcript:
                    self.logger.error(
                        "Exon is not found in rank of transcript",
                        extra={
            ...
            for line_number in self.structural_variant_entries.keys():
                ...
                if (
                    self.structural_variant_entries[line_number]["Event_Info"]
                    == "Fusion"
                ):

                    # Retrieve event info
                    site1_exon = self.structural_variant_entries[line_number][
                        "Site1_Exon"
                    ]
                    site2_exon = self.structural_variant_entries[line_number][
                        "Site2_Exon"
                    ]
                    site1_transcript = self.structural_variant_entries[line_number][
                        "Site1_Ensembl_Transcript_Id"
                    ]
                    site2_transcript = self.structural_variant_entries[line_number][
                        "Site2_Ensembl_Transcript_Id"
                    ]

                    # Validate exons
                    if (
                        site1_exon is not None
                        and site1_transcript is not None
                        and site1_transcript in self.transcript_exons_dict
                    ):
                        if self.transcript_exons_dict[site1_transcript] is not None:
                            validateExonsInTranscript(
                                self, site1_transcript, site1_exon, line_number
                            )
                    if (
                        site2_exon is not None
                        and site2_transcript is not None
                        and site2_transcript in self.transcript_exons_dict
                    ):
                        if self.transcript_exons_dict[site2_transcript] is not None:
                            validateExonsInTranscript(
                                self, site2_transcript, site2_exon, line_number
                            )
...
class PatientResourceValidator(ResourceValidator):
        ...
        for col_index, col_name in enumerate(self.cols):
            ...
            if col_name == "PATIENT_ID":
                if value not in RESOURCE_PATIENTS_WITH_SAMPLES:
                    self.logger.warning(
                        "Resource data defined for a patient with " "no samples",
                        extra={
                            "line_number": self.line_number,
                            "column_number": col_index + 1,
                            "cause": value,
    ...
    def onComplete(self):
        ...
        for patient_id in RESOURCE_PATIENTS_WITH_SAMPLES:
            if patient_id not in self.patient_id_lines:
                self.logger.warning(
                    "Missing resource data for a patient associated with " "samples",
                    extra={"cause": patient_id},
                )
...
class GenericAssayWiseFileValidator(FeaturewiseFileValidator):
    ...
    def parseFeatureColumns(self, nonsample_col_vals):
        ...
        if " " in value:
            self.logger.error(
                "Do not use space in the stable id",
                extra={
                    "line_number": self.line_number,
                    "column_number": 1,
                    "cause": nonsample_col_vals[0],
...
class GenericAssayValidator(GenericAssayWiseFileValidator):
    ...
    def checkValue(self, value, col_index):
        ...
        if stripped_value == "":
            self.logger.error(
                "Cell is empty. A response value value is expected. Use 'NA' to indicate missing values.",
                extra={
                    "line_number": self.line_number,
                    "column_number": col_index + 1,
                    "cause": value,
        ...
        try:
            ...
            self.logger.error(
                "Value cannot be interpreted as a floating point number and is not valid response value.",
                extra={
                    "line_number": self.line_number,
                    "column_number": col_index + 1,
                    "cause": value,
        ...
        if math.isnan(numeric_value):
            self.logger.error(
                "Value is NaN, therefore, not a valid response value.",
                extra={
                    "line_number": self.line_number,
                    "column_number": col_index + 1,
                    "cause": value,
        ...
        if math.isinf(numeric_value):
            self.logger.error(
                "Value is infinite and, therefore, not a valid response value.",
                extra={
                    "line_number": self.line_number,
                    "column_number": col_index + 1,
                    "cause": value,
        ...
        if numeric_value % 1 == 0 and not hasTruncSymbol:
            self.logger.warning(
                "Value has no decimals and may represent an invalid response value.",
                extra={
                    "line_number": self.line_number,
                    "column_number": col_index + 1,
                    "cause": value,
...
def process_metadata_files(
    ...
    for filename in filenames:
        ...
        meta_dictionary = cbioportal_common.parse_metadata_file(
            ...
            logger,
            study_id,
            portal_instance.genome_build,
            gene_panel_list=portal_instance.gene_panel_list,
        )
    ...
    for case in case_list_fns:
        ...
        if "case_list_category" in meta_dictionary:
            ...
            if (
                ...
                and not meta_dictionary["case_list_category"] is "other"
            ):
                logger.warning(
        ...
        if stable_id.split("_")[-1] == "sequenced":
            ...
            if len(fusion_file_sample_ids) > 0:
                if not fusion_file_sample_ids.issubset(mutation_sample_ids):
                    fusion_sample_ids_not_in_case_list = ", ".join(
                        fusion_file_sample_ids - mutation_sample_ids
                    )
                    logger.warning(
    ...
    if any(
        ...
        if not missing_gsva_file:
            ...
            if len(expression_zscores_source_stable_ids) == 0:
                ...
                if not gsva_scores_source_stable_id in list(
...
def request_from_portal_api(server_url, api_name, logger):
    ...
    if api_name == "gene-panels":
        ...
        for data_item in response.json():
            ...
            gene_panel_url = (
                service_url.strip("?pageSize=9999999")
                + "/"
                + gene_panel_id
                + "?pageSize=9999999"
            )
...
def transform_symbol_entrez_map(
    json_data, id_field="hugo_gene_symbol", values_field="entrez_gene_id"
):
    ...
    for data_item in json_data:
        ...
        if symbol not in result_dict:
            result_dict[symbol] = []
        result_dict[symbol].append(data_item["entrezGeneId"])
    return result_dict
