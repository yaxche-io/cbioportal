...
def processMRNA(originalFileName):
    ...
    if not os.path.exists(originalFileName):
        ...
        print "Preparing mRNA Data file:  %s --> %s " % (
            originalFileName,
            finalFileName,
        )
        os.system(
            BIN_DIR + "preprocess-mrna.py " + originalFileName + " > " + finalFileName
        )
...
def processCNA(originalFileName):
    ...
    if not os.path.exists(rae_original_file):
        ...
        print "Preparing CNA RAE file:  %s --> %s " % (
            rae_original_file,
            rae_final_file,
        )
        os.system(
            BIN_DIR + "preprocess-rae.py " + rae_original_file + " > " + rae_final_file
        )
...
convertCaseFile(
    DATA_DIR + "case_lists/cases_all.txt", DATA_DIR + "processed_all_cases.txt"
)
convertCaseFile(
    DATA_DIR + "case_lists/cases_sequenced.txt",
    DATA_DIR + "processed_sequenced_cases.txt",
)
