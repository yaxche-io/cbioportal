...
def get_gdata_credentials(secrets, creds, scope, force=False):
    ...
    if credentials is None or credentials.invalid or force:
        credentials = run_flow(
            flow_from_clientsecrets(secrets, scope=scope),
            storage,
...
def google_login(secrets, creds, user, pw, app_name):
    ...
    credentials = get_gdata_credentials(
        secrets, creds, ["https://spreadsheets.google.com/feeds"], False
    )
...
def insert_new_users(cursor, new_user_list):
    ...
    try:
        ...
        cursor.executemany(
            ...
            [
                (user.google_email.lower(), user.name, user.enabled)
                for user in new_user_list
            ],
        )
        for user in new_user_list:
            ...
            cursor.executemany(
                "insert into authorities values(%s, %s)",
...
def get_current_user_map(cursor):
    ...
    try:
        ...
        for row in cursor.fetchall():
            to_return[row[0].lower()] = User(
                row[0].lower(), row[0].lower(), row[1], row[2], "not_used_here"
            )
    except MySQLdb.Error, msg:
        print >> ERROR_FILE, msg
        return None
...
def get_new_user_map(
    spreadsheet, worksheet_feed, current_user_map, portal_name, mskcc_user_spreadsheet
):
    ...
    for entry in worksheet_feed.entry:
        ...
        if (
            entry.custom[STATUS_KEY].text is not None
            and entry.custom[STATUS_KEY].text.strip() == STATUS_APPROVED
        ):
...
def get_all_user_map(spreadsheet, worksheet_feed, mskcc_user_spreadsheet):
    ...
    for entry in worksheet_feed.entry:
        if spreadsheet == mskcc_user_spreadsheet:
            ...
            google_email = entry.custom[OPENID_EMAIL_KEY].text.strip().lower()
        to_return[google_email] = User(
            inst_email, google_email, "not_used", 1, "not_used"
        )
...
def get_db_connection(portal_properties):
    ...
    try:
        connection = MySQLdb.connect(
            host=portal_properties.cgds_database_host,
            port=3306,
...
def get_portal_properties(portal_properties_filename):
    ...
    # error check
    if (
        CGDS_DATABASE_HOST not in properties
        or len(properties[CGDS_DATABASE_HOST]) == 0
        or CGDS_DATABASE_NAME not in properties
        or len(properties[CGDS_DATABASE_NAME]) == 0
        or CGDS_DATABASE_USER not in properties
        or len(properties[CGDS_DATABASE_USER]) == 0
        or CGDS_DATABASE_PW not in properties
        or len(properties[CGDS_DATABASE_PW]) == 0
        or GOOGLE_ID not in properties
        or len(properties[GOOGLE_ID]) == 0
        or GOOGLE_PW not in properties
        or len(properties[GOOGLE_PW]) == 0
        or CGDS_USERS_SPREADSHEET not in properties
        or len(properties[CGDS_USERS_SPREADSHEET]) == 0
        or CGDS_USERS_WORKSHEET not in properties
        or len(properties[CGDS_USERS_WORKSHEET]) == 0
        or IMPORTER_SPREADSHEET not in properties
        or len(properties[IMPORTER_SPREADSHEET]) == 0
    ):
...
def manage_users(
    spreadsheet, cursor, worksheet_feed, portal_name, mskcc_user_spreadsheet
):
    ...
    if current_user_map is not None:
        print >> OUTPUT_FILE, "We have found %s current portal users" % len(
            current_user_map
        )
    else:
    ...
    new_user_map = get_new_user_map(
        spreadsheet,
        worksheet_feed,
        current_user_map,
        portal_name,
        mskcc_user_spreadsheet,
    )
...
def update_user_authorities(
    spreadsheet, cursor, worksheet_feed, portal_name, mskcc_user_spreadsheet
):

    # get map of current portal users
    print >> OUTPUT_FILE, "Getting list of current portal users from spreadsheet"
    all_user_map = get_new_user_map(
        spreadsheet, worksheet_feed, {}, portal_name, mskcc_user_spreadsheet
    )
    ...
    for user in all_user_map.values():
        ...
        try:
            cursor.executemany(
                "insert into authorities values(%s, %s)",
                [
                    (user.google_email, authority)
                    for authority in worksheet_authorities - db_authorities
                ],
            )
        except MySQLdb.Error, msg:
            print >> ERROR_FILE, msg
...
def add_unknown_users_to_spreadsheet(
    client, cursor, spreadsheet, worksheet, mskcc_user_spreadsheet
):
    ...
    google_spreadsheet_user_map = get_all_user_map(
        spreadsheet, worksheet_feed, mskcc_user_spreadsheet
    )
    ...
    for email in portal_db_user_map.keys():
        if (
            email.endswith(MSKCC_EMAIL_SUFFIX)
            and email not in google_spreadsheet_user_map
        ):
            user = portal_db_user_map[email]
            print >> OUTPUT_FILE, user.name
            def_authorities = (
                DEFAULT_AUTHORITIES + ";" + email[0 : email.index("@")].upper()
            )
            ...
            if "/" in user.name:
                ...
                row = {
                    ...
                    MSKCC_EMAIL_KEY: user.inst_email,
                    FULLNAME_KEY: user_name_parts[0],
                    LAB_PI_KEY: user_name_parts[1],
                    STATUS_KEY: STATUS_APPROVED,
                    AUTHORITIES_KEY: def_authorities,
                }
            else:
                row = {
                    TIMESTAMP_KEY: current_time,
                    MSKCC_EMAIL_KEY: user.inst_email,
                    FULLNAME_KEY: user.name,
                    STATUS_KEY: STATUS_APPROVED,
                    AUTHORITIES_KEY: def_authorities,
                }
...
def get_email_parameters(google_spreadsheet, client):
    ...
    email_worksheet_feed = get_worksheet_feed(
        client, google_spreadsheet, IMPORTER_WORKSHEET
    )
    for entry in email_worksheet_feed.entry:
        if (
            entry.custom[SUBJECT_KEY].text is not None
            and entry.custom[BODY_KEY].text is not None
        ):
...
def get_portal_name_map(google_spreadsheet, client):
    ...
    access_control_worksheet_feed = get_worksheet_feed(
        client, google_spreadsheet, ACCESS_CONTROL_WORKSHEET
    )
    for entry in access_control_worksheet_feed.entry:
        if (
            entry.custom[PORTAL_NAME_KEY] is not None
            and entry.custom[SPREADSHEET_NAME_KEY] is not None
        ):
            portal_name[entry.custom[SPREADSHEET_NAME_KEY].text.strip()] = entry.custom[
                PORTAL_NAME_KEY
            ].text.strip()
...
def main():
    ...
    if (
        secrets_filename == ""
        or creds_filename == ""
        or properties_filename == ""
        or send_email_confirm == ""
        or (send_email_confirm != "true" and send_email_confirm != "false")
    ):
        usage()
        sys.exit(2)
    ...
    client = google_login(
        secrets_filename,
        creds_filename,
        portal_properties.google_id,
        portal_properties.google_pw,
        sys.argv[0],
    )

    portal_name_map, mskcc_user_spreadsheet = get_portal_name_map(
        portal_properties.google_importer_spreadsheet, client
    )
    ...
    for google_spreadsheet in google_spreadsheets:
        if not google_spreadsheet == "":
            ...
            worksheet_feed = get_worksheet_feed(
                client, google_spreadsheet, portal_properties.google_worksheet
            )

            # the 'guts' of the script
            new_user_map = manage_users(
                google_spreadsheet,
                cursor,
                worksheet_feed,
                portal_name_map[google_spreadsheet],
                mskcc_user_spreadsheet,
            )
