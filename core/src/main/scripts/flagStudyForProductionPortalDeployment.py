...
def flag_study_for_production_portal_deployment(
    client, worksheet_feed, cancer_study_id, remove_from_triage
):
...
def usage():
    print >> OUTPUT_FILE, (
        "flagStudyForProductionPortalDeployment.py --secrets-file [google secrets.json] --creds-file [oauth creds filename] --google-id --google-password "
...
def main():
    ...
    client = google_login(
        secrets_filename, creds_filename, google_id, google_password, sys.argv[1]
    )
    worksheet_feed = get_worksheet_feed(client, google_spreadsheet, google_worksheet)
    flag_study_for_production_portal_deployment(
        client, worksheet_feed, cancer_study_id, remove_from_triage
    )
