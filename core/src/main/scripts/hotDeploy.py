...
class Credentials(object):
    def __init__(self, cgds_database_user, cgds_database_pw, bitly_user, bitly_key):
...
def get_portal_credentials(credentials):
    ...
    for line in credentials_file:
        ...
        if (
            len(line) == 0
            or line.startswith("#")
            or line.startswith(CGDS_USERS_SPREADSHEET)
        ):
            continue
        # store name/value
        property = line.split("=")
    ...
    # error check
    if (
        CGDS_DATABASE_USER not in properties
        or len(properties[CGDS_DATABASE_USER]) == 0
        or CGDS_DATABASE_PW not in properties
        or len(properties[CGDS_DATABASE_PW]) == 0
        or BITLY_USER not in properties
        or len(properties[BITLY_USER]) == 0
        or BITLY_KEY not in properties
        or len(properties[BITLY_KEY]) == 0
    ):
...
def build_war(portal_credentials, portal_properties):
    ...
    for line in portal_properties_file:
        ...
        if line.startswith(CGDS_DATABASE_USER):
            new_line = "%s=%s\n" % (
                CGDS_DATABASE_USER,
                portal_credentials.cgds_database_user,
            )
        elif line.startswith(CGDS_DATABASE_PW):
            new_line = "%s=%s\n" % (
                CGDS_DATABASE_PW,
                portal_credentials.cgds_database_pw,
            )
...
def usage():
    print >> OUTPUT_FILE, (
        "hotDeploy.py --credentials [credentials] "
...
def main():
    ...
    try:
        opts, args = getopt.getopt(
            sys.argv[1:],
            "",
            ["credentials=", "portal-properties=", "rev=", "host=", "user="],
        )
    ...
    # deploy
    print >> OUTPUT_FILE, "Deploying war file to: %s@%s:%s" % (
        user,
        host,
        WAR_FILE_DEST,
    )
