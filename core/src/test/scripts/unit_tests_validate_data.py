...
def setUpModule():
    ...
    PATIENTS_WITH_SAMPLES = set(
        "TEST-PAT{}".format(num) for num in list(range(1, 10)) if num != 8
    )
    mutation_sample_ids = ["TCGA-A1-A0SB-01", "TCGA-A1-A0SD-01"]
    logger = logging.getLogger(__name__)
    # parse mock API results from a local directory
    PORTAL_INSTANCE = validateData.load_portal_info(
        "test_data/api_json_unit_tests", logger, offline=True
    )
...
class LogBufferTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.output_handler.setFormatter(
            cbioportal_common.LogfileStyleFormatter("test_data/")
        )
...
class ColumnOrderTestCase(DataFileTestCase):
    ...
    def test_column_order_validation_ClinicalValidator(self):
        ...
        record_list = self.validate(
            "data_clin_order1.txt", validateData.SampleClinicalValidator
        )
        # we expect no errors or warnings
        self.assertEqual(0, len(record_list))
        # if the file has another order, this is also OK:
        record_list = self.validate(
            "data_clin_order2.txt", validateData.SampleClinicalValidator
        )
...
class UniqueColumnTestCase(PostClinicalDataFileTestCase):
    ...
    def test_uniquecolumns_are_accepted(self):
        ...
        record_list = self.validate(
            "data_unique_column_test.txt",
            UniqueColumnTestCase.DummyFeaturewiseFileValidator,
        )
...
class ClinicalColumnDefsTestCase(PostClinicalDataFileTestCase):
    ...
    def test_correct_definitions(self):
        ...
        record_list = self.validate(
            "data_clin_coldefs_correct.txt", validateData.PatientClinicalValidator
        )
    ...
    def test_invalid_definitions(self):
        ...
        record_list = self.validate(
            "data_clin_coldefs_invalid_priority.txt",
    ...
    def test_lowercase_attribute(self):
        ...
        record_list = self.validate(
            "data_clin_coldefs_lowercase_attribute.txt",
            validateData.PatientClinicalValidator,
        )
    ...
    def test_hardcoded_attributes(self):
        ...
        self.logger.setLevel(logging.WARNING)
        record_list = self.validate(
            "data_clin_coldefs_hardcoded_attrs.txt",
            validateData.PatientClinicalValidator,
        )
        self.assertEqual(len(record_list), 3)
    ...
    def test_banned_attribute(self):
        ...
        self.logger.setLevel(logging.WARNING)
        record_list = self.validate(
            "data_clin_coldefs_banned_attribute.txt",
            validateData.PatientClinicalValidator,
        )
        ...
        self.assertEqual(record.column_number, 6)
        self.assertIn(
            "MUTATION_COUNT and FRACTION_GENOME_ALTERED are calculated in cBioPortal",
            record.getMessage(),
        )
...
class ClinicalValuesTestCase(DataFileTestCase):
    ...
    def test_sample_twice_in_one_file(self):
        ...
        self.logger.setLevel(logging.ERROR)
        record_list = self.validate(
            "data_clin_repeated_sample.txt", validateData.SampleClinicalValidator
        )
        self.assertEqual(len(record_list), 1)
    ...
    def test_tcga_sample_twice_in_one_file(self):
        ...
        self.logger.setLevel(logging.WARNING)
        record_list = self.validate(
            "data_clin_repeated_tcga_sample.txt", validateData.SampleClinicalValidator
        )
        self.assertEqual(len(record_list), 1)
        record = record_list.pop()
    ...
    def test_sample_with_invalid_characters_in_sample_id(self):
        ...
        self.logger.setLevel(logging.WARNING)
        record_list = self.validate(
            "data_clin_wrong_ids.txt", validateData.SampleClinicalValidator
        )
        ...
        self.assertIn(
            "can only contain letters, numbers, points, underscores and/or hyphens",
            record.getMessage(),
        )
        # last one:
        record = record_list.pop()
...
class PatientAttrFileTestCase(PostClinicalDataFileTestCase):
    ...
    def test_patient_without_samples(self):
        ...
        self.logger.setLevel(logging.WARNING)
        record_list = self.validate(
            "data_clin_patient_without_samples.txt",
            validateData.PatientClinicalValidator,
        )
        self.assertEqual(len(record_list), 1)
        record = record_list.pop()
    ...
    def test_patient_without_attributes(self):
        ...
        self.logger.setLevel(logging.WARNING)
        record_list = self.validate(
            "data_clin_missing_patient.txt", validateData.PatientClinicalValidator
        )
        self.assertEqual(len(record_list), 1)
        record = record_list.pop()
        self.assertEqual(record.levelno, logging.WARNING)
        self.assertFalse(
            hasattr(record, "line_number"), "logrecord is about a specific line"
        )
    ...
    def test_hardcoded_attr_values(self):
        ...
        self.logger.setLevel(logging.WARNING)
        record_list = self.validate(
            "data_clin_hardcoded_attr_vals.txt", validateData.PatientClinicalValidator
        )
        self.assertEqual(len(record_list), 6)
        record_iterator = iter(record_list)
    ...
    def test_patient_with_invalid_characters_in_patient_id(self):
        ...
        self.logger.setLevel(logging.WARNING)
        record_list = self.validate(
            "data_clin_wrong_patient_id.txt", validateData.PatientClinicalValidator
        )
    ...
    def test_date_in_nondate_column(self):
        ...
        self.logger.setLevel(logging.ERROR)
        record_list = self.validate(
            "data_clin_date_in_nondate_column.txt",
            validateData.PatientClinicalValidator,
        )
...
class TimelineValuesDataValidationTest(DataFileTestCase):
    ...
    def test_start_date_validation_TimelineValidator(self):
        ...
        self.logger.setLevel(logging.ERROR)
        record_list = self.validate(
            "data_timeline_invalid_start_date.txt", validateData.TimelineValidator
        )
        self.assertEqual(len(record_list), 2)
...
class CancerTypeFileValidationTestCase(DataFileTestCase):
    ...
    def test_new_cancer_type(self):
        ...
        record_list = self.validate(
            "data_cancertype_lung.txt", validateData.CancerTypeValidator
        )
    ...
    def test_cancer_type_missing_column(self):
        ...
        self.logger.setLevel(logging.ERROR)
        record_list = self.validate(
            "data_cancertype_missing_color_col.txt", validateData.CancerTypeValidator
        )
    ...
    def test_cancer_type_missing_value(self):
        ...
        self.logger.setLevel(logging.ERROR)
        record_list = self.validate(
            "data_cancertype_blank_color_col.txt", validateData.CancerTypeValidator
        )
    ...
    def test_cancer_type_undefined_parent(self):
        ...
        self.logger.setLevel(logging.ERROR)
        record_list = self.validate(
            "data_cancertype_undefined_parent.txt", validateData.CancerTypeValidator
        )
        self.assertEqual(len(record_list), 1)
        record = record_list.pop()
        self.assertEqual(record.levelno, logging.ERROR)
    ...
    def test_cancer_type_invalid_color(self):
        ...
        self.logger.setLevel(logging.ERROR)
        record_list = self.validate(
            "data_cancertype_invalid_color.txt", validateData.CancerTypeValidator
        )
        self.assertEqual(len(record_list), 1)
        record = record_list.pop()
        self.assertEqual(record.levelno, logging.ERROR)
    ...
    def test_cancer_type_matching_portal(self):
        ...
        record_list = self.validate(
            "data_cancertype_confirming_existing.txt", validateData.CancerTypeValidator
        )
        # expecting only status messages about the file being validated
        self.assertEqual(len(record_list), 3)
        for record in record_list:
    ...
    def test_cancer_type_disagreeing_with_portal(self):
        ...
        self.logger.setLevel(logging.ERROR)
        record_list = self.validate(
            "data_cancertype_redefining.txt", validateData.CancerTypeValidator
        )
    ...
    def test_cancer_type_defined_twice(self):
        ...
        self.logger.setLevel(logging.ERROR)
        record_list = self.validate(
            "data_cancertype_lung_twice.txt", validateData.CancerTypeValidator
        )
...
class GeneIdColumnsTestCase(PostClinicalDataFileTestCase):
    ...
    def test_both_name_and_entrez(self):
        ...
        record_list = self.validate(
            "data_cna_genecol_presence_both.txt", validateData.CNADiscreteValidator
        )
        # expecting only status messages about the file being validated
        self.assertEqual(len(record_list), 3)
        for record in record_list:
    ...
    def test_name_only(self):
        ...
        self.logger.setLevel(logging.WARNING)
        record_list = self.validate(
            "data_cna_genecol_presence_hugo_only.txt", validateData.CNADiscreteValidator
        )
        # expecting 1 warning
        self.assertEqual(len(record_list), 1)
        for record in record_list:
    ...
    def test_entrez_only(self):
        ...
        record_list = self.validate(
            "data_cna_genecol_presence_entrez_only.txt",
    ...
    def test_neither_name_nor_entrez(self):
        ...
        record_list = self.validate(
            "data_cna_genecol_presence_neither.txt", validateData.CNADiscreteValidator
        )
    ...
    def test_both_name_and_entrez_but_invalid_hugo(self):
        ...
        self.logger.setLevel(logging.WARNING)
        record_list = self.validate(
            "data_cna_genecol_presence_both_invalid_hugo.txt",
            validateData.CNADiscreteValidator,
        )
    ...
    def test_both_name_and_entrez_but_hugo_starts_with_integer(self):
        ...
        self.logger.setLevel(logging.ERROR)
        record_list = self.validate(
            "data_cna_genecol_presence_both_invalid_hugo_integer.txt",
            validateData.CNADiscreteValidator,
    ...
    def test_both_name_and_entrez_but_invalid_entrez(self):
        ...
        self.logger.setLevel(logging.WARNING)
        record_list = self.validate(
            "data_cna_genecol_presence_both_invalid_entrez.txt",
            validateData.CNADiscreteValidator,
        )
    ...
    def test_both_name_and_entrez_but_invalid_couple(self):
        ...
        self.logger.setLevel(logging.WARNING)
        record_list = self.validate(
            "data_cna_genecol_presence_both_invalid_couple.txt",
            validateData.CNADiscreteValidator,
        )
        # expecting two error messages:
        self.assertEqual(len(record_list), 2)
    ...
    def test_name_only_but_invalid(self):
        ...
        self.logger.setLevel(logging.WARNING)
        record_list = self.validate(
            "data_cna_genecol_presence_hugo_only_invalid.txt",
            validateData.CNADiscreteValidator,
        )
        # expecting two warning messages:
        self.assertEqual(len(record_list), 3)
    ...
    def test_name_only_but_ambiguous(self):
        ...
        self.logger.setLevel(logging.WARNING)
        record_list = self.validate(
            "data_cna_genecol_presence_hugo_only_ambiguous.txt",
            validateData.CNADiscreteValidator,
        )
    ...
    def test_entrez_only_but_invalid(self):
        ...
        self.logger.setLevel(logging.WARNING)
        record_list = self.validate(
            "data_cna_genecol_presence_entrez_only_invalid.txt",
            validateData.CNADiscreteValidator,
        )
        # expecting two warning messages:
        self.assertEqual(len(record_list), 2)
    ...
    def test_unambiguous_hugo_also_used_as_alias(self):
        ...
        self.logger.setLevel(logging.WARNING)
        record_list = self.validate(
            "data_cna_genecol_presence_hugo_only_possible_alias.txt",
            validateData.CNADiscreteValidator,
        )
        # expecting one error message
        self.assertEqual(len(record_list), 2)
    ...
    def test_blank_column_heading(self):
        ...
        self.logger.setLevel(logging.ERROR)
        record_list = self.validate(
            "data_cna_blank_heading.txt", validateData.CNADiscreteValidator
        )
        self.assertEqual(len(record_list), 4)
        for record in record_list:
            self.assertEqual(record.levelno, logging.ERROR)
    ...
    def test_cytoband_column(self):
        ...
        self.logger.setLevel(logging.WARNING)
        record_list = self.validate(
            "data_cna_cytoband.txt", validateData.CNADiscreteValidator
        )
...
class FeatureWiseValuesTestCase(PostClinicalDataFileTestCase):
    ...
    def test_repeated_gene(self):
        ...
        self.logger.setLevel(logging.WARNING)
        record_list = self.validate(
            "data_cna_duplicate_gene.txt", validateData.CNADiscreteValidator
        )
    ...
    def test_invalid_discrete_cna(self):
        ...
        self.logger.setLevel(logging.ERROR)
        record_list = self.validate(
            "data_cna_invalid_values.txt", validateData.CNADiscreteValidator
        )
    ...
    def test_valid_rppa(self):
        ...
        self.logger.setLevel(logging.DEBUG)
        record_list = self.validate(
            "data_rppa_valid.txt", validateData.ProteinLevelValidator
        )
        # expecting only status messages about the file being validated
        self.assertEqual(len(record_list), 3)
        for record in record_list:
    ...
    def test_invalid_rppa(self):
        ...
        self.logger.setLevel(logging.ERROR)
        record_list = self.validate(
            "data_rppa_invalid_values.txt", validateData.ProteinLevelValidator
        )
    ...
    def test_repeated_rppa_entry(self):
        ...
        self.logger.setLevel(logging.WARNING)
        record_list = self.validate(
            "data_rppa_duplicate_entries.txt", validateData.ProteinLevelValidator
        )
    ...
    def test_na_gene_in_rppa(self):
        ...
        self.logger.setLevel(logging.WARNING)
        record_list = self.validate(
            "data_rppa_na_gene.txt", validateData.ProteinLevelValidator
        )
    ...
    def test_gsva_range_gsva_scores(self):
        ...
        self.logger.setLevel(logging.ERROR)
        record_list = self.validate(
            "data_gsva_scores_outrange.txt", validateData.GsvaScoreValidator
        )
    ...
    def test_range_gsva_pvalues(self):
        ...
        self.logger.setLevel(logging.ERROR)
        record_list = self.validate(
            "data_gsva_pvalues_outrange.txt", validateData.GsvaPvalueValidator
        )
    ...
    def test_missing_column_gsva(self):
        ...
        record_list1 = self.validate(
            "data_gsva_pvalues_missing_column.txt", validateData.GsvaPvalueValidator
        )

        record_list2 = self.validate(
            "data_gsva_scores_missing_column.txt", validateData.GsvaScoreValidator
        )
    ...
    def test_missing_row_gsva(self):
        ...
        ### Error should appear when the second file is validated
        record_list1 = self.validate(
            "data_gsva_pvalues_missing_row.txt", validateData.GsvaPvalueValidator
        )

        record_list2 = self.validate(
            "data_gsva_scores_missing_row.txt", validateData.GsvaScoreValidator
        )
        self.assertEqual(len(record_list1), 0)
        ...
        for record in record_list2:
            self.assertEqual(record.levelno, logging.ERROR)
        self.assertEqual(
            "Gene sets column in score and p-value file are not equal. The same set of gene sets should be used in the score and p-value files for this study. Please ensure that all gene set id's of one file are present in the other gene set data file.",
            record.getMessage(),
        )
        return
...
class MultipleDataFileValidatorTestCase(unittest.TestCase):
    def feature_id_is_accepted(self):
        mockval = Mock()
        validateData.MultipleDataFileValidator.parseFeatureColumns(
...
class ContinuousValuesTestCase(PostClinicalDataFileTestCase):
    ...
    def test_invalid_methylation(self):
        ...
        self.logger.setLevel(logging.ERROR)
        record_list = self.validate(
            "data_methylation_invalid_values.txt",
...
class MutationsSpecialCasesTestCase(PostClinicalDataFileTestCase):
    def test_normal_samples_list_in_maf(self):
        ...
        self.logger.setLevel(logging.ERROR)
        record_list = self.validate(
            "mutations/data_mutations_invalid_norm_samples.maf",
            validateData.MutationsExtendedValidator,
            {
                "normal_samples_list": "TCGA-BH-A18H-10,"
                "TCGA-B6-A0RS-10,"
                ""  # TCGA-BH-A0HP-10
                "TCGA-BH-A18P-11, "
                "TCGA-C8-A138-10"
    ...
    def test_missing_aa_change_column(self):
        ...
        self.logger.setLevel(logging.ERROR)
        record_list = self.validate(
            "mutations/data_mutations_missing_aa_change_column.maf",
            validateData.MutationsExtendedValidator,
        )
    ...
    def test_maf_file_checks(self):
        ...
        record_list = self.validate(
            "mutations/data_mutations_test_variant_types.maf",
            validateData.MutationsExtendedValidator,
            None,
            True,
            True,
        ...
        self.assertEqual(record.line_number, 2)
        self.assertIn(
            "Variant_Type indicates insertion, but difference in Start_Position and End_Position does "
            "not equal to 1 or the length or the Reference_Allele.",
            record.getMessage(),
        )
        ...
        self.assertEqual(record.line_number, 3)
        self.assertIn(
            "Variant_Type indicates insertion, but length of Reference_Allele is bigger than the length "
            "of the Tumor_Seq_Allele1 and/or 2 and therefore indicates deletion.",
            record.getMessage(),
        )
        ...
        self.assertEqual(record.line_number, 4)
        self.assertIn(
            "Variant_Type indicates deletion, but the difference between Start_Position and End_Position "
            "are not equal to the length of the Reference_Allele.",
            record.getMessage(),
        )
        ...
        self.assertEqual(record.line_number, 5)
        self.assertIn(
            "Variant_Type indicates deletion, but length of Reference_Allele is smaller than the length "
            "of Tumor_Seq_Allele1 and/or Tumor_Seq_Allele2, indicating an insertion.",
            record.getMessage(),
        )
        ...
        self.assertEqual(record.line_number, 7)
        self.assertIn(
            "Variant_Type indicates a SNP, but Reference_Allele, Tumor_Seq_Allele1 "
    ...
    def test_special_allele_cases(self):
        ...
        # Test file is a variant on test_output.custom_isoforms.maf from vcf2maf
        record_list = self.validate(
            "mutations/data_mutations_check_special_cases_allele.maf",
            validateData.MutationsExtendedValidator,
            None,
            True,
            True,
        ...
        self.assertEqual(record.line_number, 2)
        self.assertIn(
            "All Values in columns Reference_Allele, Tumor_Seq_Allele1 and Tumor_Seq_Allele2 are equal.",
            record.getMessage(),
        )
    ...
    def test_validation_status_MAF_checks(self):
        ...
        record_list = self.validate(
            "mutations/data_mutations_validation_status.maf",
            validateData.MutationsExtendedValidator,
            None,
            True,
            True,
        ...
        self.assertEqual(record.line_number, 2)
        self.assertIn(
            "Validation Status is valid, but Validation Allele columns are empty.",
            record.getMessage(),
        )
        ...
        self.assertEqual(record.line_number, 4)
        self.assertIn(
            "When Validation_Status is invalid the Tumor_Validation_Allele and Match_Norm_"
            "Validation_Allele columns should be equal.",
            record.getMessage(),
        )
        ...
        self.assertEqual(record.line_number, 5)
        self.assertIn(
            "Validation Status is invalid, but Validation_Method is not defined.",
            record.getMessage(),
        )
        ...
        self.assertEqual(record.line_number, 6)
        self.assertIn(
            "When Validation_Status is valid and Mutation_Status is Germline, the Tumor_Validation_Allele "
            "should be equal to the Match_Norm_Validation_Allele.",
            record.getMessage(),
        )
        # expect error for incorrect Validation Allele Columns when Mutation_Status is Germline
        record = next(record_iterator)
        self.assertEqual(record.line_number, 7)
        self.assertIn(
            "When Validation_Status is valid and Mutation_Status is Somatic, the Match_Norm_Validation_Allele"
    ...
    def test_warning_for_missing_SWISSPROT(self):
        ...
        self.logger.setLevel(logging.WARNING)
        record_list = self.validate(
            "mutations/data_mutations_missing_swissprot.maf",
            validateData.MutationsExtendedValidator,
        )
        ...
        # check if both messages come from printDataInvalidStatement:
        self.assertIn(
            "swissprot column is recommended", record_list[0].getMessage().lower()
        )
    ...
    def test_invalid_swissprot_identifier_type(self):
        ...
        self.assertIsNone(
            meta_dictionary["meta_file_type"],
            "metadata file was not rejected as invalid",
        )
    ...
    def test_silent_mutation_skipped(self):
        ...
        self.logger.setLevel(logging.INFO)
        record_list = self.validate(
            "mutations/data_mutations_some_silent.maf",
            validateData.MutationsExtendedValidator,
            extra_meta_fields={"swissprot_identifier": "name"},
        )
    ...
    def test_alternative_notation_for_intergenic_mutation(self):
        ...
        self.logger.setLevel(logging.INFO)
        record_list = self.validate(
            "mutations/data_mutations_silent_alternative.maf",
            validateData.MutationsExtendedValidator,
            extra_meta_fields={"swissprot_identifier": "name"},
        )
        ...
        self.assertIn(
            "no entrez gene id or gene symbol provided",
            record_list[1].getMessage().lower(),
        )
        self.assertEqual(record_list[1].levelno, logging.ERROR)
        ...
        self.assertIn(
            ...
            record_list[4].getMessage().lower(),
        )
    ...
    def test_customized_variants_skipped(self):
        ...
        validateData.MutationsExtendedValidator.SKIP_VARIANT_TYPES = [
            "5'Flank",
            "Frame_Shift_Del",
            "Frame_Shift_Ins",
        ]
    ...
    def test_isValidGenePosition(self):
        ...
        self.assertEqual(record.levelno, logging.ERROR)
        self.assertIn(
            "The end position of this variant is not " "an integer", record.getMessage()
        )
    ...
    def test_absence_custom_values_columns_when_custom_annotation_columns(self):
        ...
        self.logger.setLevel(logging.ERROR)
        record_list = self.validate(
            "mutations/data_mutations_absence_custom_values_columns_when_custom_annotation_columns.maf",
            validateData.MutationsExtendedValidator,
            extra_meta_fields={"swissprot_identifier": "name"},
        )
        # we expect 5 ERRORs :
    ...
    def test_absence_custom_annotation_columns_when_custom_values_columns(self):
        ...
        self.logger.setLevel(logging.ERROR)
        record_list = self.validate(
            "mutations/data_mutations_absence_custom_annotation_columns_when_custom_values_columns.maf",
            validateData.MutationsExtendedValidator,
            extra_meta_fields={"swissprot_identifier": "name"},
        )
        # we expect 5 ERRORs :
        ...
        self.assertIn(
            "please, fill the annotation column", record_list[2].getMessage().lower()
        )
        self.assertEqual(record_list[0].levelno, logging.ERROR)
        self.assertIn(
            "please, fill the annotation column", record_list[3].getMessage().lower()
        )
        self.assertEqual(record_list[0].levelno, logging.ERROR)
        self.assertIn(
            "please, fill the annotation column", record_list[4].getMessage().lower()
        )
        self.assertEqual(record_list[0].levelno, logging.ERROR)
    ...
    def test_empty_custom_annotation_fields(self):
        ...
        self.logger.setLevel(logging.ERROR)
        record_list = self.validate(
            "mutations/data_mutations_empty_custom_annotation_fields.maf",
            validateData.MutationsExtendedValidator,
            extra_meta_fields={"swissprot_identifier": "name"},
        )
        # we expect 2 ERRORs :
    ...
    def test_warning_more_than_10_types_in_driver_class(self):
        ...
        self.assertIn(
            "cbp_driver_tiers contains more than 10 different tiers.",
            record_list[2].getMessage().lower(),
        )
        self.assertEqual(record_list[2].levelno, logging.WARNING)
    ...
    def test_annotation_more_than_80_characters_in_custom_annotation_columns(self):
        ...
        self.logger.setLevel(logging.ERROR)
        record_list = self.validate(
            "mutations/data_mutations_more_than_80_characters_in_custom_annotation_columns.maf",
            validateData.MutationsExtendedValidator,
            extra_meta_fields={"swissprot_identifier": "name"},
        )
        # we expect 1 ERROR :
        ...
        self.assertIn(
            "columns do not support annotations longer than 80 characters",
            record_list[0].getMessage().lower(),
        )
        self.assertEqual(record_list[0].levelno, logging.ERROR)
    ...
    def test_not_supported_custom_driver_annotation_values(self):
        ...
        self.logger.setLevel(logging.ERROR)
        record_list = self.validate(
            "mutations/data_mutations_not_supported_custom_driver_annotation_values.maf",
            validateData.MutationsExtendedValidator,
            extra_meta_fields={"swissprot_identifier": "name"},
        )
        # we expect 1 ERROR :
        ...
        self.assertIn(
            'only "putative_passenger", "putative_driver", "na", "unknown" and "" (empty) are allowed.',
            record_list[0].getMessage().lower(),
        )
        self.assertEqual(record_list[0].levelno, logging.ERROR)
    ...
    def test_custom_driver_column_more_than_50_characters(self):
        ...
        self.logger.setLevel(logging.ERROR)
        record_list = self.validate(
            "mutations/data_mutations_custom_tiers_column_more_than_50_characters.maf",
            validateData.MutationsExtendedValidator,
            extra_meta_fields={"swissprot_identifier": "name"},
        )
        # we expect 1 ERROR :
        ...
        self.assertIn(
            "does not support values longer than 50 characters",
            record_list[0].getMessage().lower(),
        )
        self.assertEqual(record_list[0].levelno, logging.ERROR)
    ...
    def test_validation_verification_status(self):
        ...
        self.logger.setLevel(logging.WARNING)
        record_list = self.validate(
            "mutations/data_mutations_vs.maf",
            validateData.MutationsExtendedValidator,
            extra_meta_fields={"swissprot_identifier": "name"},
        )
        ...
        self.assertEqual(record.cause, "---")
        self.assertEqual(
            record.getMessage(), "Value in 'Validation_Status' not in MAF format"
        )
        ...
        self.assertEqual(
            record.getMessage(), "Value in 'Verification_Status' not in MAF format"
        )
    ...
    def test_validation_ms(self):
        ...
        self.logger.setLevel(logging.WARNING)
        record_list = self.validate(
            "mutations/data_mutations_ms.maf",
            validateData.MutationsExtendedValidator,
            extra_meta_fields={"swissprot_identifier": "name"},
        )
        # we expect 1 warning
        ...
        self.assertEqual(
            record_list[0].getMessage(), "Mutation_Status value is not in MAF format"
        )
    ...
    def test_mutation_not_loaded_ms(self):
        ...
        self.logger.setLevel(logging.INFO)
        record_list = self.validate(
            "mutations/data_mutations_not_loaded_ms.maf",
            validateData.MutationsExtendedValidator,
            extra_meta_fields={"swissprot_identifier": "name"},
        )
    ...
    def test_mutation_invalid_utf8(self):
        ...
        self.logger.setLevel(logging.ERROR)
        record_list = self.validate(
            "mutations/data_mutations_invalid_utf8.maf",
            validateData.MutationsExtendedValidator,
        )
        # we expect 1 ERROR:
        self.assertEqual(len(record_list), 1)
...
class FusionValidationTestCase(PostClinicalDataFileTestCase):
    ...
    def test_duplicate_line(self):
        ...
        self.logger.setLevel(logging.WARNING)
        record_list = self.validate(
            "data_fusions_duplicate_entry.txt", validateData.FusionValidator
        )

        self.assertEqual(len(record_list), 1)
        self.assertIn(
            "duplicate entry in fusion data", record_list[0].getMessage().lower()
        )
...
class StructuralVariantValidationTestCase(PostClinicalDataFileTestCase):
    ...
    def test_missing_columns(self):
        ...
        self.logger.setLevel(logging.ERROR)
        record_list = self.validate(
            "data_structural_variants_missing_columns.txt",
        ...
        self.assertEqual(
            'Fusion event requires "Site1_Exon" and "Site2_Exon" columns',
            record.message,
        )
        ...
        self.assertEqual(1, record.line_number)
        self.assertEqual(
            'Fusion event requires "Site1_Ensembl_Transcript_Id" and "Site2_Ensembl_Transcript_Id" '
            "columns",
            record.message,
        )
    ...
    def test_missing_values(self):
        ...
        self.logger.setLevel(logging.ERROR)
        record_list = self.validate(
            "data_structural_variants_missing_values.txt",
            validateData.StructuralVariantValidator,
        )
    ...
    def test_transcript_not_in_genome_nexus(self):
        ...
        self.logger.setLevel(logging.ERROR)
        record_list = self.validate(
            "data_structural_variants_transcript_not_in_genome_nexus.txt",
            validateData.StructuralVariantValidator,
        )
        ...
        self.assertIn(
            "Ensembl transcript not found in Genome Nexus.", record.getMessage()
        )
...
class SegFileValidationTestCase(PostClinicalDataFileTestCase):
    ...
    def test_invalid_ref_genome_hg18(self):
        ...
        self.assertEqual(
            record.message,
            "Genomic position beyond end of chromosome (chr19:0-59128983)",
        )
...
class GenePanelMatrixValidationTestCase(PostClinicalDataFileTestCase):
    ...
    def test_duplicate_sample(self):
        ...
        self.logger.setLevel(logging.ERROR)
        record_list = self.validate(
            "data_gene_matrix_duplicate_sample.txt",
...
class StudyCompositionTestCase(LogBufferTestCase):
    ...
    def test_double_cancer_type_file(self):
        ...
        self.assertEqual(
            filenames_in_cause_string, {"cancer_type_luad.txt", "cancer_type_lung.txt"}
        )
...
class CaseListDirTestCase(PostClinicalDataFileTestCase):
    ...
    def test_duplicated_stable_id(self):
        ...
        validateData.processCaseListDirectory(
            "test_data/case_lists_duplicated", "brca_tcga_pub", self.logger
        )
        record_list = self.get_log_records()
        self.assertEqual(len(record_list), 1)
        record = record_list.pop()
        ...
        self.assertTrue(
            record.cause.startswith("brca_tcga_pub_all"),
    ...
    def test_duplicated_sample_id(self):
        ...
        self.logger.setLevel(logging.WARNING)
        validateData.processCaseListDirectory(
            "test_data/case_lists_duplicated_sampleid", "brca_tcga_pub", self.logger
        )
        record_list = self.get_log_records()
        self.assertEqual(len(record_list), 1)
        record = record_list.pop()
    ...
    def test_invalid_category(self):
        ...
        self.logger.setLevel(logging.ERROR)
        validateData.processCaseListDirectory(
            "test_data/case_lists_invalid_category", "brca_tcga_pub", self.logger
        )
        record_list = self.get_log_records()
        self.assertEqual(len(record_list), 1)
        record = record_list.pop()
    ...
    def test_duplicate_category(self):
        ...
        self.logger.setLevel(logging.WARNING)
        validateData.processCaseListDirectory(
            "test_data/case_lists_duplicate_category", "brca_tcga_pub", self.logger
        )
        record_list = self.get_log_records()
        self.assertEqual(len(record_list), 1)
        record = record_list.pop()
...
class MetaFilesTestCase(LogBufferTestCase):
    ...
    def test_unnecessary_and_wrong_stable_id(self):
        ...
        validateData.process_metadata_files(
            "test_data/study_metastableid", PORTAL_INSTANCE, self.logger, False, False
        )
        record_list = self.get_log_records()
    ...
    def test_exceed_maximum_length_meta_attribute_value(self):
        ...
        record = record_list.pop()
        self.assertEqual(
            "The maximum length of the 'short_name' value is 64", record.getMessage()
        )
    ...
    def test_invalid_pmid_values(self):
        ...
        self.assertEqual(
            "29625048, 29596782, 29622463A, 29617662 29625055, 29625050", record.cause
        )
    ...
    def test_show_profile_setting_for_cna(self):
        ...
        record = record_list.pop()
        self.assertEqual(
            "The 'show_profile_in_analysis_tab' setting must be 'false', as this is only applicable for CNA data of the DISCRETE type.",
            record.getMessage(),
        )
...
class HeaderlessClinicalDataValidationTest(PostClinicalDataFileTestCase):
    ...
    def test_headerless_clinical_sample(self):
        ...
        self.assertTrue(final_record.getMessage().lower().startswith("read 13 lines"))
...
class DataFileIOTestCase(PostClinicalDataFileTestCase):
    ...
    def test_missing_datafile(self):
        ...
        self.logger.setLevel(logging.ERROR)
        record_list = self.validate(
            "filename-that-does-not-exist.txt", validateData.ContinuousValuesValidator
        )
        self.assertEqual(len(record_list), 1)
        record = record_list.pop()
        self.assertEqual(record.levelno, logging.ERROR)
...
class ResourceDefinitionWiseTestCase(PostClinicalDataFileTestCase):
    def test_resource_definition_missing_resourceId(self):
        ...
        self.logger.setLevel(logging.ERROR)
        record_list = self.validate(
            "data_resource_definition_missing_resourceId.txt",
...
class ResourceWiseTestCase(PostClinicalDataFileTestCase):
    def test_resource_is_not_url(self):
        ...
        self.logger.setLevel(logging.ERROR)
        record_list = self.validate(
            "data_resource_is_not_url.txt", validateData.ResourceValidator
        )
    ...
    def test_sample_resource_should_have_definition(self):
        ...
        self.logger.setLevel(logging.ERROR)
        record_list = self.validate(
            "data_resource_sample_valid.txt", validateData.SampleResourceValidator
        )
    ...
    def test_sample_resource_has_definition_in_different_type(self):
        ...
        validateData.RESOURCE_DEFINITION_DICTIONARY = {
            "PATHOLOGY_SLIDE": ["SAMPLE", "PATIENT"]
        }
        self.logger.setLevel(logging.WARNING)
        record_list = self.validate(
            "data_resource_sample_valid.txt", validateData.SampleResourceValidator
        ...
        self.assertEqual(record.levelno, logging.WARNING)
        self.assertIn(
            "sample resource has been used by more than one RESOURCE_TYPE",
            record.getMessage(),
        )
    ...
    def test_sample_resource_has_duplication(self):
        self.logger.setLevel(logging.ERROR)
        record_list = self.validate(
            "data_resource_sample_duplicate.txt", validateData.SampleResourceValidator
        )
    ...
    def test_patient_resource_should_have_definition(self):
        ...
        self.logger.setLevel(logging.ERROR)
        record_list = self.validate(
            "data_resource_patient_valid.txt", validateData.PatientResourceValidator
        )
    ...
    def test_patient_resource_has_definition_in_different_type(self):
        ...
        self.assertEqual(record.levelno, logging.WARNING)
        self.assertIn(
            "patient resource has been used by more than one RESOURCE_TYPE",
            record.getMessage(),
        )
    ...
    def test_patient_resource_has_duplication(self):
        ...
        self.logger.setLevel(logging.ERROR)
        record_list = self.validate(
            "data_resource_patient_duplicate.txt", validateData.PatientResourceValidator
        )
    ...
    def test_study_resource_should_have_definition(self):
        ...
        self.logger.setLevel(logging.ERROR)
        record_list = self.validate(
            "data_resource_study_valid.txt", validateData.StudyResourceValidator
        )
    ...
    def test_study_resource_has_definition_in_different_type(self):
        ...
        self.assertEqual(record.levelno, logging.WARNING)
        self.assertIn(
            "study resource has been used by more than one RESOURCE_TYPE",
            record.getMessage(),
        )
    ...
    def test_study_resource_has_duplication(self):
        self.logger.setLevel(logging.ERROR)
        record_list = self.validate(
            "data_resource_study_duplicate.txt", validateData.StudyResourceValidator
        )
